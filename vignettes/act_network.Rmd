...
This will just set your credentials for the current R session. If you wish to use the same credentials after closing/reopening R, use the `temporary = FALSE` argument:
...
What you might first want to do is check out all of the data extract files on offer. Make sure you include the server that you're querying ("act").
...
The most important thing, however, is downloading those files! Let's grab the parquet files and create a mini-database for ourselves.
